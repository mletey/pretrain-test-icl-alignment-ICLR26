...
def parse_args():
    ...
    p.add_argument("--test_power", type=float, default=0.0)
...
def main():
    ...
    numsamples = 500
    CSV_COLUMNS = ["d", "kappa", "alpha", "tau", "train_power", "test_power", "test_m", "test_s", "seed"]
    loss_func = optax.squared_error

    Ctest = np.diag(np.array([(j + 1) ** -args.test_power for j in range(args.d)])); Ctest = (Ctest/np.trace(Ctest))*args.d
    ...
    append_row_csv(args.savedirectory, {
        ...
        "test_power": float(args.test_power),
        "test_m": test_m,
        "test_s": test_s,
        "seed": args.seed,
    }, CSV_COLUMNS)
